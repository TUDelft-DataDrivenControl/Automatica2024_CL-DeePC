...
\begin{align}\label{eq:PE_corUE}
    ...
    \end{bmatrix}^\top > 0,
\end{align}
which is equivalent to the positive definiteness of an experimental correlation matrix that can be decomposed into its constituent components. By considering Schur complements it follows that \eqref{eq:PE_corUE} is satisfied if both \eqref{eq:PE_corUE2} and $\widehat{\Sigma}_{u,u} > 0$. Note that the former condition is satisfied by assumption and that
