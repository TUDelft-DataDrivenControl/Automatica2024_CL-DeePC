...
\begin{lem}[\cite{Willems2005}, Cor.~2(iii)]
    If the input sequence $\{u_k\}_{k=i}^{i+\epsilon+q-2}$ of a discrete \ac{LTI} system without noise is persistently exciting of order $\epsilon+n$ then the matrix $\left[X_{i,1,q}^\top\;U_{i,\epsilon,q}^\top\right]^\top$ is full row rank.
\end{lem}
