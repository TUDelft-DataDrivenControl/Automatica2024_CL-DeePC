...
\end{align}
in which $\widehat{\Sigma}_{u,e}=U_{i,p+s+n,q-n}E_{i,p+s+n,q-n}^\top$, and likewise for $\widehat{\Sigma}_{u,u}$ and $\widehat{\Sigma}_{e,e}$. Note that by Definition~\ref{def:PE}, \eqref{eq:PE_corU} is satisfied if the input is persistently exciting of order $p+s+n$.

Hence, if the aforementioned persistency of excitation conditions~\eqref{eq:PE_corU} and~\eqref{eq:PE_corUE2} are met and the \ac{LTI} system is controllable then $\mathfrak{D}$ from \eqref{eq:RBD2} is full row rank. As a result, by the Rouch\'{e}-Capelli theorem there then exists a vector $g$ such that
\begin{align}\label{eq:Dg}
    \mathfrak{D} g =
    \begin{bmatrix}
        x_{\hat{i}}^\top & \datavec{u}{\hat{i},p}^\top & \datavec{u}{\hat{i}_p,s}^\top & \datavec{e}{\hat{i},p}^\top & \datavec{e}{\hat{i}_p,s}^\top
    \end{bmatrix}^\top.
\end{align}
With reference to \eqref{eq:}, premultiplying both sides of~\eqref{eq:Dg} by $\mathfrak{B}$ from~\eqref{RBD2}
% \begin{align}
%     \mathfrak{D}g=\begin{bmatrix}
%         x_\hat{i}^\top & \datavec{u}{\hat{i},p}^\top & \datavec{u}{\hat{i}_p,s}^\top & \datavec{e}{\hat{i},p}^\top & \datavec{e}{\hat{i}_p,s}
%     \end{bmatrix}^\top.
% \end{align}
