...
\begin{align}\label{eq:Dg}
    ...
    \end{bmatrix}.
\end{align}
Equation~\eqref{eq:Theorem1} is obtained by sequential application of \eqref{eq:Dg} for $k={\hat{i},\dots,\hat{i}+f-1}$ without the top and bottom matrix equations. As a result the future outputs resemble predictions and $G=\left[g_1\;g_2\;\cdots\;g_f\right]$. Since $\Phi_{i,s,N}$ has more columns then rows with the condition $N\geq(p+s+l)(r+l)+n$ thereThis proves statement~\ref{claim:G_exists}).
%
