...
\begin{thm}\label{theorem:main_result}
    ...
    \end{align}
    \begin{enumerate}
        \item there exists a solution $G$\label{claim:G_exists}
...
\begin{align}\label{eq:Dg}
    ...
    \end{bmatrix}.
\end{align}
This is written more compactly without the top and bottom matrix equations for consecutive time indices $k={\hat{i},\dots,\hat{i}+f-1}$ with $G$ containing the vectors $\{g_{k-\hat{i}+1}\}_{k=\hat{i}}^{\hat{i}+f-1}$ by \eqref{eq:Theorem1}. , this proves statement~(\ref{claim:G_exists}).
