...
\begin{thm}\label{theorem:main_result}
    ...
    \begin{enumerate}
        ...
        \item[2)] with a corresponding output predictor $\widehat{Y}_{\hat{i}_p,s,f}$ that is uniquely specified by the past data and noise realization%that uniquely specifies an output predictor $\widehat{Y}_{\hat{i}_p,s,f}$
        \item[3)] that is asymptotically unbiased with $p\rightarrow\infty$ if $s=1$.
    \end{enumerate}
...
\begin{align}\label{eq:Dg}
    ...
    \end{bmatrix}.
\end{align}
Without the top and bottom matrix equations this is written more compactly for consecutive time indices $k={\hat{i},\dots,\hat{i}+f-1}$ as given by \eqref{eq:Theorem1}, in which $G$ contains $\{g_{k-\hat{i}+1}\}_{k=\hat{i}}^{\hat{i}+f-1}$. This proves statement~\ref{claim:G_exists}).
