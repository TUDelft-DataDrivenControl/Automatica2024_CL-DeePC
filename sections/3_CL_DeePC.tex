...
\begin{thm}\label{theorem:main_result}
    Consider the minimal discrete non-deterministic \ac{LTI} system given by~\eqref{eqn:SS_innovation}. If $p\geq\ell$, the sequence of inputs $\{u_k\}_{k=i}^{i+\bar{N}-1}$ of length $\bar{N}=p+s+N-1$ is persistently exciting of order $p+s+n$, and sample correlations such that%
