...
\end{align*}
with $\mathfrak{R}\in\mathbb{R}^{sl\times n+(p+s)(r+l)+sl}$, and $\mathfrak{BD}$ as indicated for brevity. Similarly, decomposing $\mathfrak{BD}$ into a matrix of exogenous inputs and initial states $\mathfrak{D}$ and a matrix to describe their effects $\mathfrak{B}$ using \eqref{eq:DataEq1} for $Y_{k,p,q}$ and $Y_{k_p,s,q}$ obtains
\begin{align}
    \mathfrak{R}
    \underbrace{\begin{bmatrix}
        I_n      & 0      & 0       & 0 & 0\\
