...
\begin{thm}\label{theorem:main_result}
    ...
    \begin{enumerate}
        ...
        \item[2)] that uniquely specifies an output predictor $\widehat{Y}_{\hat{i}_p,s,f}$
...
\begin{align}\label{eq:Dg}
    ...
    \end{bmatrix}.
\end{align}
This is written more compactly without the top and bottom matrix equations for consecutive time indices $k={\hat{i},\dots,\hat{i}+f-1}$ such that $G$ contains $\{g_{k-\hat{i}+1}\}_{k=\hat{i}}^{\hat{i}+f-1}$ as given by \eqref{eq:Theorem1}. This proves statement~\ref{claim:G_exists}.
