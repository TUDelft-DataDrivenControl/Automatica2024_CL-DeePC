...
\begin{align*}
    \underbrace{\begin{bmatrix}-\Gamma_s \tilde{A}^p & -L_s & I_{sl}&-\mathcal{H}_s\end{bmatrix}}_{= M_\mathfrak{R}}
    \begin{bmatrix}
        X_{k,1,q}\\
        \Phi_{k,s,q}\\
        Y_{k_p,s,q}\\
...
\end{align*}
with $M_\mathfrak{R}\in\mathbb{R}^{sl\times n+(p+s)(r+l)+sl}$ defined as indicated for brevity. Similarly, decomposing the right hand side matrix into contributions from exogenous inputs and initial states using \eqref{eq:DataEq1} for $Y_{k,p,q}$ and $Y_{k_p,s,q}$ obtains
\begin{align}
    M_\mathfrak{R}
    ...
    \end{bmatrix}=\mathcal{O},
\end{align}
with $\varepsilon_1=\Gamma_s(\tilde{A}^p+\tKp{y}\Gamma_p)$, $\varepsilon_2=\Gamma_s(\tKp{u}+\tKp{y}\mathcal{T}_p^\mathrm{u})$, and $\varepsilon_3=\Gamma_s\tKp{y}\mathcal{H}_p$. By inspection, the range of the underbraced matrix $M_\mathfrak{B}$ is equal to the nullspace of $M_\mathfrak{R}$: $\mathcal{R}(M_\mathfrak{B})=\mathcal{N}(M_\mathfrak{B})$. If the matrix post-multiplying $M_\mathfrak{B}$ is full row rank then the range of this product is also equal to the nullspace of $M_\mathfrak{R}$.
