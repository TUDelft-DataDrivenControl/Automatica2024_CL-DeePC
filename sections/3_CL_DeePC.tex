...
\begin{align}\label{eq:PE_cov1}
    ...
    \end{bmatrix}^\top > 0,
\end{align}
which is equivalent to the positive definiteness of an experimental correlation matrix that can be decomposed into its constituent components. Defining  $\widehat{\Sigma}_{u,e}=U_{i,p+s+n,q-n}E_{i,p+s+n,q-n}^\top$, and likewise $\widehat{\Sigma}_{u,u}$ and $\widehat{\Sigma}_{e,e}$, by a Schur decomposition it can be shown that \eqref{eq:PE_cov1} is satisfied if
\begin{align}
    \widehat{\Sigma}_{e,e} &> 0,\\
    \widehat{\Sigma}_{u,u} &> \widehat{\Sigma}_{u,e} \widehat{\Sigma}_{e,e}\inv \widehat{\Sigma}_{u,e}^\top.
\end{align}
