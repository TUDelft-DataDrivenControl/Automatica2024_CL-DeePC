...
\begin{thm}\label{theorem:main_result}
    ...
    \begin{enumerate}
        ...
        \item[2)] with a corresponding output predictor $\widehat{Y}_{\hat{i}_p,s,f}$ that
        \begin{enumerate}
            \item is uniquely determined by past noise and input-output data if $\Phi_{i,s,N}$ is square, and%that uniquely specifies an output predictor
...
\begin{align}\label{eq:Dg}
    ...
    \end{bmatrix}.
\end{align}
Without the top and bottom matrix equations this is written more compactly for consecutive time indices $k={\hat{i},\dots,\hat{i}+f-1}$ as given by \eqref{eq:Theorem1}, in which $G$ contains the vectors $\{g_{k-\hat{i}+1}\}_{k=\hat{i}}^{\hat{i}+f-1}$. This proves statement~\ref{claim:G_exists}).
