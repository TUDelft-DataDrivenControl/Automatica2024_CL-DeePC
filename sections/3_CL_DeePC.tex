...
\begin{align}\label{eq:Dg}
    ...
    \end{bmatrix}.
\end{align}
Equation~\eqref{eq:Theorem1} is obtained by sequential application of \eqref{eq:Dg} for $k={\hat{i},\dots,\hat{i}+f-1}$ such that $G=\left[g_1\;g_2\;\cdots\;g_f\right]$ without the top and bottom matrix equations. As a result the future outputs resemble predictions. Since the condition $N\geq(p+s+n)(r+l)+n$ ensures that $\Phi_{i,s,N}$ has more columns then rows, there are multiple solutions to $G$ in \eqref{eq:Theorem1}. This proves statement~\ref{claim:G_exists}).
%
