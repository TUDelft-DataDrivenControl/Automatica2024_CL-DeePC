...
\begin{align}\label{eq:PE_corUE}
    ...
    \end{bmatrix}^\top > 0,
\end{align}
which is equivalent to the positive definiteness of an experimental correlation matrix that can be decomposed into its constituent components. By considering Schur complements it follows that \eqref{eq:PE_corUE} is satisfied if both \eqref{eq:PE_corUE2} and $\widehat{\Sigma}_{u,u} > 0$. Note that both conditions are satisfied by assumption, with the latter condition being implied by Definition~\ref{def:PE}.
\begin{align}%see also https://www.cis.upenn.edu/~jean/schur-comp.pdf
    \label{eq:PE_corU}
\end{align}
are satisfied. Note that \eqref{eq:PE_corU} is satisfied by extension of Definition~\ref{def:PE} and the assumption that the input is persistently exciting of order $p+s+n$.
