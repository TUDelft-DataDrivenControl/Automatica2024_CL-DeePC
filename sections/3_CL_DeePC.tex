...
\end{align}
which is equivalent to the positive definiteness of an experimental correlation matrix that can be decomposed into its constituent components. Defining  $\widehat{\Sigma}_{u,e}=U_{i,p+s+n,q-n}E_{i,p+s+n,q-n}^\top$, and likewise $\widehat{\Sigma}_{u,u}$ and $\widehat{\Sigma}_{e,e}$, \eqref{eq:PE_cov1} yields
