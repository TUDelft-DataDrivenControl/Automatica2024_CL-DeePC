...
\begin{thm}\label{theorem:main_result}
    Consider the minimal discrete non-deterministic \ac{LTI} system given by~\eqref{eqn:SS_innovation}. If the sequence of inputs $\{u_k\}_{k=i}^{i+\bar{N}-1}$ of length $\bar{N}=p+s+N-1$ is persistently exciting of order $p+s+n$, and %
    ...
    \text{with }&&\widehat{\Sigma}_{e,e}&=E_{i,p+s+n,N-n}E_{i,p+s+n,N-n}^\top,\notag\\
    &&\widehat{\Sigma}_{u,e}&=U_{i,p+s+n,N-n}E_{i,p+s+n,N-n}^\top,\notag\\
    &&\widehat{\Sigma}_{u,u}&=U_{i,p+s+n,N-n}U_{i,p+s+n,N-n}^\top\notag
    \end{align}
    in which $\widehat{\Sigma}_{e,e}=E_{i,p+s+n,N-n}E_{i,p+s+n,N-n}^\top$, $\widehat{\Sigma}_{u,e}=U_{i,p+s+n,N-n}E_{i,p+s+n,N-n}^\top$, $\widehat{\Sigma}_{u,u}=U_{i,p+s+n,N-n}U_{i,p+s+n,N-n}^\top$
