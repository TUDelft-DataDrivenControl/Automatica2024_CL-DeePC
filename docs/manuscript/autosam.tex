...
Improved data availability and the complexity that is often associated with system modelling have sparked interest in data-driven predictive control approaches like Data-enabled Predictive Control (DeePC). Often applied adaptively, such methods then employ data obtained during closed-loop operation. Unfortunately the resulting correlation between inputs and preceding outputs prevents DeePC from employing a consistent data-driven predictor. The principal aim of this paper is the development of closed-loop data-enabled predictive control (CL-DeePC), which addresses this issue in a behavioural framework using instrumental variables (IVs) to provide a structural means of noise mitigation. Moreover, an efficient sequential implementation is presented that reduces the number of optimization variables and reveals an equivalence between CL-DeePC and closed-loop subspace predictive control (CL-SPC). Simulations illustrate the closed-loop identification problem and demonstrate the resulting superior reference tracking performance of CL-DeePC when compared to DeePC.%
% {\color{white}
