...
Improved data availability and the complexity that is often associated with system modelling have sparked interest in data-driven predictive control approaches like \ac{DeePC}. Often applied adaptively, such methods then employ data obtained during closed-loop operation. Unfortunately, the resulting correlation between inputs and preceding noise prevents \ac{DeePC} from employing a consistent data-driven predictor. The principal aim of this paper is the development of \ac{CL-DeePC}, which addresses this issue in a behavioural framework using \ac{IVs} to provide a structural means of noise mitigation. Moreover, an efficient sequential implementation is presented that reduces the number of optimization variables and reveals an equivalence between \ac{CL-DeePC} and \ac{CL-SPC}. Simulations illustrate the closed-loop identification problem and demonstrate the resulting superior reference tracking performance of \ac{CL-DeePC} when compared to \ac{DeePC}.%
% {\color{white}
