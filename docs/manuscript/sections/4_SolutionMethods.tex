...
The use of such an efficient method can be understood by comparing the number of (unequal) optimization variables (in red) in respectively Fig.~\ref{fig:CL-DeePC} and Fig.~\ref{fig:regular-DeePC}. Both with and without \ac{IVs}, the number of such independent optimization variables used in \ac{CL-DeePC} scales worse with $f$ than is the case for \ac{DeePC}. This is inherent to the formulation depicted by Fig.~\ref{fig:CL-DeePC} whereby multiple vectors $g_{(\cdot)}$ are used.

As a first step to reduce the number of optimization variables, in line with Remark~\ref{rem:square_min_opt_var}, consider only choices of $\mathcal{Z}$ that render $\Psi_{i,1,N}\mathcal{Z}^\top$ square and invertible. Using~\eqref{eq:G_sols}, the predictor of~\eqref{eq:TheoremIV} may then be written as
...
\end{align}
with $\hat{\Sigma}_{yz}=Y_{i_p,1,f}\mathcal{Z}^\top$, $\hat{\Sigma}_{\psi z}=\Psi_{i,1,N}\mathcal{Z}^\top$.
