...
\begin{assum}\label{assum:relative-rates}
    ...
    \end{align}
\end{assum}
For the system $\mathcal{S}$ from \secref{sec:sys_model} with $\rho=\max|\emph{eig}(\tilde{A})|<1$ this assumption ensures that errors due to mis-specification of the initial condition are $o_P(1/\sqrt{N})$ (and may therefore be neglected), and that sample correlations approach true correlations that are well-defined~\citep{Bauer2002,Chiuso2006}.
...
\end{assum}
This assumption entails that used past input-output data is sufficiently persistently exciting, which is an important requirement for many data-driven applications. %~\citep{vanWaarde2020}.
