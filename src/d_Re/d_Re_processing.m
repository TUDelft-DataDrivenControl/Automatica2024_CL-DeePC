...
Re_all = cellfun(@(x) str2double(x{1}),matches);
Re_all = sort(Re_all,2,'ascend');
...
quantiles = 0.1:0.2:0.9;%[0.1 0.25 0.5 0.75 0.9];
n_quant = length(quantiles); idx_median = ceil(n_quant/2);
[costs_DeePC1,costs_CLDeePC1] = deal(nan(n_quant,num_n)); % quantiles w/ median of cost
[costs_DeePC2,costs_CLDeePC2] = deal(nan(1,num_n));       % mean cost
costs_Oracle1 = costs_DeePC1;
costs_Oracle2 = costs_DeePC2;

% pre-determine some constants
Re = Re_all(1);
fn = strcat('Re_',num2str(Re),'.mat');
a = load(fn,'results');
...
width1 = 8.4*1.5;
scale1 = width1/pos1(3);
set(fig1,'Position',[pos1(1:2),width1,scale1*pos1(4)])
[ff0,ff1,ff2] = deal(cell(1,idx_median-1));
FaceAlpha = 0.6/(idx_median-1);
for idx = 1:idx_median-1
    if num_c > 2
        ff0{idx}=fill([Re_all fliplr(Re_all)],[costs_Oracle1(idx,:),  fliplr(costs_Oracle1(end-idx+1,:))],color{3},HandleVisibility='off');
...
if num_c > 2
    l0 = semilogy(Re_all,costs_Oracle1(idx_median,:),  Color=color{3},LineWidth=1.5); hold on;
    l01= semilogy(Re_all,costs_Oracle2,Color=color{3}, LineStyle='--',LineWidth=1.5);
end
l1 = semilogy(Re_all,costs_DeePC1(idx_median,:),   Color=color{1},LineWidth=1.5);
l11= semilogy(Re_all,costs_DeePC2,Color=color{1},  LineStyle='--',LineWidth=1.5);
l2 = semilogy(Re_all,costs_CLDeePC1(idx_median,:), Color=color{2},LineWidth=1.5);
l21= semilogy(Re_all,costs_CLDeePC2,Color=color{2},LineStyle='--',LineWidth=1.5);
% plot(Re_all,sqrt(Re_all)./NormRef,'k-',LineWidth=1.5);
if num_c > 2
